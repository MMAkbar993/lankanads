 ...
 *
 * `initialAd` comes from the server component that renders this, so the ad
 * is already on screen in the first paint (and in the HTML search engines
 * see). `identifier` is only used as a fallback for the legacy client-side
 * route, where nothing was fetched server-side.
 */
export default function AdDetailView({ initialAd = null, identifier = "" }) {
  ...
  const adFromList = initialAd
    ? null
    : ads.find(
        (item) => item._id === identifier || item.adId === identifier
      );

  const [fetchedAd, setFetchedAd] = useState(null);
  const [notFound, setNotFound] = useState(false);

  const ad = initialAd || adFromList || fetchedAd;

  // Only set once the server confirms the new count — until then the count
  // that came with the ad is shown.
  const [liveViewsCount, setLiveViewsCount] = useState(null);
  const viewsCount = liveViewsCount ?? (ad?.viewsCount || ad?.views || 0);

  // Nothing was supplied by the server and it isn't in the browsed list
  // (direct reload of a legacy link, etc.) — fetch it directly.
  useEffect(() => {
    if (initialAd || adFromList || !identifier) return;

    dispatch(getPublicAdById(identifier)).then((result) => {
  ...
  }, [initialAd, adFromList, identifier, dispatch]);
  ...
  useEffect(() => {
    ...
    dispatch(incrementAdView(ad._id)).then((result) => {
      if (incrementAdView.fulfilled.match(result)) {
        setLiveViewsCount(result.payload.viewsCount || 0);
      }
    });
  }, [dispatch, ad?._id]);

  if (!ad && identifier && !notFound) {
    return (
  ...
  // Live Cam ads keep the poster's plain phone number hidden — only the
  // messaging apps they explicitly opted into are shown.
  const isLiveCam = ad.category === "Live Cam";
  const hasWhatsapp = Boolean(ad.socialAvailability?.whatsapp);
  const hasTelegram = Boolean(ad.socialAvailability?.telegram);
      ...
      <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--white)] shadow-sm">
        ...
        <div className="p-4 sm:p-5">
          ...
          <div className="mt-8 space-y-5 text-[15px] leading-7 text-gray-600">
            ...
            {/* Repeated below the description too — ad descriptions here can
                run very long (full price lists etc.), so a reader shouldn't
                have to scroll back to the top just to make contact. */}
            <ContactButtons
              isLiveCam={isLiveCam}
              phone={phone}
              whatsappNumber={whatsappNumber}
...
// The poster's contact options for this ad: phone call, WhatsApp, Telegram.
// Live Cam ads never show the plain phone/Call button — only the messaging
// apps the poster explicitly ticked, matching how those listings are
// conventionally run.
function ContactButtons({
  ...
  const telegramDigits = phone.replace(/\D/g, "");
  ...
  return (
    <div className={className}>
      {!isLiveCam && (
        <a
          href={`tel:${phone}`}
          className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-md border border-[var(--primary)] py-1 text-[18px] font-normal text-[var(--primary)] transition hover:bg-[var(--primary)] hover:text-white"
        >
          <Phone size={16} />
          Call {phone}
        </a>
      ...
      {hasWhatsapp && (
        <a
          ...
          className={`flex w-full cursor-pointer items-center justify-center gap-2 rounded-md border border-green-500 py-1 text-[18px] font-normal text-green-600 transition hover:bg-green-500 hover:text-white ${
            isLiveCam ? "" : "mt-3"
          }`}
        >
          <WhatsappIcon />
      ...
      {hasTelegram && (
        <a
          href={`https://t.me/+${telegramDigits}`}
          target="_blank"
          rel="noopener noreferrer"
          className={`flex w-full cursor-pointer items-center justify-center gap-2 rounded-md border border-sky-500 py-1 text-[18px] font-normal text-sky-600 transition hover:bg-sky-500 hover:text-white ${
            isLiveCam && !hasWhatsapp ? "" : "mt-3"
          }`}
        >
          <TelegramIcon />
          {phone}
...
// lucide-react in this version has no Send/Telegram glyph pinned to a stable
// name, so the brand mark is inlined the same way as WhatsApp above.
function TelegramIcon() {
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <path d="M9.417 15.181l-.397 5.584c.568 0 .814-.244 1.109-.537l2.663-2.545 5.518 4.041c1.012.564 1.725.267 1.998-.931L23.98 3.723c.32-1.478-.533-2.055-1.518-1.7L1.115 10.077c-1.437.559-1.417 1.365-.245 1.72l5.518 1.723L18.9 6.15c.548-.36 1.047-.16.637.2z" />
    </svg>
  );
}
